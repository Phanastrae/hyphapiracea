    ...
    public void tick(long time) {
        if(this.lastUpdateTime != time) {
            ...
            if (this.needsRecalculation) {
                ...
                //timer.stop();
                //HyphaPiracea.LOGGER.info("circuit " + this + " took " + timer.micro() + " microseconds to recalculate");
            }
        }
    }
    ...
    public static void solveMatrix(AugmentedMatrix augmentedMatrix) {
        ...
        for(int j = 0; j < augmentedMatrix.getColumns(); j++) {
            ...
            if(maxIndex != j) {
                ...
                //HyphaPiracea.LOGGER.info("swap rows " + maxIndex + " and " + j);
                //HyphaPiracea.LOGGER.info(augmentedMatrix.toString());
            }
            ...
            if(vj != 1) {
                ...
                //HyphaPiracea.LOGGER.info("multiply row "+ j + " by 1 / " + vj);
                //HyphaPiracea.LOGGER.info(augmentedMatrix.toString());
            }
            ...
            for(int i = 0; i < augmentedMatrix.getRows(); i++) {
                if(i != j) {
                    ...
                    if(Math.abs(vi) > 1E-8) {
                        ...
                        //HyphaPiracea.LOGGER.info("add " + -vi + " copies of row " + j + " to row " + i);
                        //HyphaPiracea.LOGGER.info(augmentedMatrix.toString());
                    }
                }
            }
